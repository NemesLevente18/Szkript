...
def front_back(a, b):
    if (len(a) % 2) == 0:
        startA = a[: int(len(a) / 2)]
        endA = a[int(len(a) / 2) :]
    else:
        startA = a[: int(len(a) / 2) + 1]
        endA = a[int(len(a) / 2) + 1 :]

    if (len(b) % 2) == 0:
        startB = b[: int(len(b) / 2)]
        endB = b[int(len(b) / 2) :]
    else:
        startB = b[: int(len(b) / 2) + 1]
        endB = b[int(len(b) / 2) + 1 :]

    s = startA + startB + endA + endB
    return s
